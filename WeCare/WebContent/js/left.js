 ...
 (function($) {
	...
	**/
	...
	$(function(){
		...
		/*$('#date_picker').html("test1");*/
		
		/*$('#date_picker').html("test2");
				...
				$.ajax({
					...
					success: function(data) {
						...
						if(!$('#search-str').val()) {
							...
							$.each(values, function(index, topic) {
								/*alert(index + " " + topic.topicName);*/
								
								$('#search-results ul').append(
									$('<li>').append(
										$('<a>').attr('href', 'TopicDetails?tid=' + topic.topicID).append(
											topic.topicName + " ").append(
												$("<span class='info-small'>").append(': ' + topic.uid))));
								
								/*$("#search-results").getNiceScroll().resize();*/
							});//each
						}
					},//success
					error: function(jqXHR, textStatus, errorThrown) {
						...
						$('#search-results ul').append(
							$('<li>').append(
								$('<span style="color:red; text-align: center; width: 215;">').append(jqXHR.responseText)));
					}//error
				});//ajax
			}
		);//keyup
		...
		$('#event-when').appendDtpicker({
			"dateFormat": "DD/MM/YYYY hh:mm",
			"closeOnSelected": true
		});
		
		$('#add-event-but').click(function() {
			var container = $('#add-event'); 
		...
		//datetime picker using the plugin
		/*$('#event-when').datetimepicker();*/
		
		//scroll bar
		/*$("#search-results").css('display', 'none');*/
		/*$("#search-results").getNiceScroll().resize();*/
		/*$("#search-results").niceScroll({ autohidemode: true });*/
		...
		//email popup configuration and ajax call
		$('#mail-link img').click(function() {
			var container = $('#email'); 
			if(!container.is(':visible')) {
				container.fadeIn();
			}
		...
		$('#email form').submit(function(event) {
			...
			$('#overlay-back, #sending-email').fadeIn();
			...
			$.ajax({
				...
				success: function(response) {
					$('#email form')[0].reset();
					alert(response);
					$('#overlay-back, #sending-email').fadeOut();
					$('#email').fadeOut('slow');
				},
				error: function(err) {
					alert(err);
		...
		//clearing the search results on document click
		$(document).mouseup(function (e) {
			...
			if (!container.is(e.target) // if the target of the click isn't the container...
					...
					&& !$('#date-picker').is(e.target)
					&& $('#date-picker').has(e.target).length === 0 ) // ... nor a descendant of the container
			{
				$('#add-event').fadeOut();
			} else {
		    	$('#add-event').fadeIn();
